...
func (con *Connect) SelctSign(event leafBot.Event) int {
	con.Exist(event)
	var sign Sign
	err := Db.Get(&sign, "select * from sign where userId=?", strconv.Itoa(event.UserId))
	if err != nil {
		log.Println(err)
		return 0
	} else {
...
func (con *Connect) Exist(event leafBot.Event) {
	...
	err := Db.Get(&sign, "select * from sign where userId=?", strconv.Itoa(event.UserId))
	if err != nil {
		_, _ = Db.Exec("insert into sign (userId, num, card, day) VALUES (?,?,?,?)",
...
func (con *Connect) IsSign(event leafBot.Event) bool {
	con.Exist(event)
	var sign Sign
	_ = Db.Get(&sign, "select * from sign where userId=?", strconv.Itoa(event.UserId))
	if sign.Day == time.Now().Day() {
		return true
	} else {
		con.Update(2, event)
		Db.Exec("update sign set day = ? where userId = ?", time.Now().Day(), strconv.Itoa(event.UserId))
		return false
	}
}
...
func (con *Connect) Update(n int, event leafBot.Event) {
	con.Exist(event)
	if n > 0 {
		_, err := Db.Exec("update sign set num = num + ? where userId = ?", n, strconv.Itoa(event.UserId))
		if err != nil {
			log.Println(err)
		}
	} else {
